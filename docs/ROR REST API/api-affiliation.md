...
Additional possibilities that might match the string are also included in results, listed in descending order by confidence `score`. Note that we **do not recommend** using the confidence `score` to select matches; use the `chosen:true` value instead.
