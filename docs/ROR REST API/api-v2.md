...
Five types of relationships are supported: `parent`, `child`, `related`, `predecessor`, and `successor`. An organization can have multiple relationships, but each relationship must be expressed as one of these relationship types. Note that inverse and corresponding relationships are significantly affected by the type of relationship and by record [status](doc:ror-data-structure#status).
...
A record with a status of `inactive` or `withdrawn` may have one or more Successor organizations listed in its relationships. Successor relationships indicate that another organization continues the work of an organization that has become inactive or has been withdrawn. See [relationships](doc:ror-data-structure#relationships) for more information.
