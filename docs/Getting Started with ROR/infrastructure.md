...
While ROR currently runs on AWS infrastructure, it could be easily ported to another provider because ROR has carefully captured all configuration as Terraform code in Github. This allows a developer to easily understand all the components needed to run ROR services, along with their configuration.

Security
The ROR dataset does not contain sensitive data, which minimizes data privacy/security risks. ROR software also does not provide user accounts or other access-controlled services that could result in exposed credentials or unauthorized access. The main security risk that ROR faces is unauthorized access to external accounts and services used by ROR, such as AWS, Github and Docker, which could result in service interruption, loss of access to accounts needed to run ROR services, and/or financial and legal issues due to unauthorized use of ROR resources/accounts. ROR mitigates this risk through the following approaches:
...
* Using a secure password storage/sharing service (1Password)
