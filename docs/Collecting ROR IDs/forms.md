...
The `ror_display` name can be in any language. If you prefer to select a name in a particular language, select the value for the name where the `type` is "label" that contains the appropriate two-letter <Anchor label="ISO 639" target="_blank" href="https://www.loc.gov/standards/iso639-2/php/code_list.php">ISO 639</Anchor>  language code in the `lang` element.
